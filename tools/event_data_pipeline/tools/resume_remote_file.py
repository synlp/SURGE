...
def main() -> int:
    parser = argparse.ArgumentParser(description="Resume an authorized SFTP download and verify its final size/checksum")
    ...
    if args.local_path.exists():
        raise RuntimeError("final local file already exists; refusing to overwrite")
    ...
    try:
        ...
        try:
            ...
            with sftp.open(args.remote_path, "rb") as source, partial.open("ab") as target:
                source.seek(resumed_from)
